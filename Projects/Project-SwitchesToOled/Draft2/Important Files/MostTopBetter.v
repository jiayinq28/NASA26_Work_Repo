...
// only necessary if and only if you want live string changes (btnL) and switch inputs (sw)
// ==== END OF EDIT ====    
    ...
    output       oled_vbat,
    output       oled_vdd,
//    output oled_cs,// used in Pmod OLED implementation    (CG: I am not entirely too sure what this refers to)
    output [7:0] led
);
    //state machine codes
    localparam Idle        = 0;
...
//        endcase
    ...
    wire col_in_new = (write_base_addr[6:3] <= 4'd7);  // cols 0-7 are within the 8-char snapshot
    ...
    always @(*) begin
        case (row)
            0: // Row 0 has "Binary :"
                write_ascii_data <= 8'hff & (str1 >> ({3'b0, (str1len - 1 - write_base_addr[6:3])} << 3));//index string parameters as str[x]
            1: // Row 1 -> binary string
                write_ascii_data = col_in_new ? char_from_row(get_new_binary_row,  col) : 8'h20;
            2: // Row 2 has "Base Ten :"
                write_ascii_data <= 8'hff & (str3 >> ({3'b0, (str3len - 1 - write_base_addr[6:3])} << 3));
            3: // Row 3 -> decimal string
                write_ascii_data = col_in_new ? char_from_row(get_new_decimal_row, col) : 8'h20;
