...
module top(
    ...
    input [7:0] sw,     // switches read by the Bluespec-generated mkOledTop module
...
//    output oled_cs,// used in Pmod OLED implementation
    ...
    //text to be displayed, EDIT to your liking
    localparam str1=" Binary :       ", str1len=16;
    localparam str3=" Base Ten :     ", str3len=16;
    // str2 and str4 are now driven dynamically by mkOledTop below
    // (binary switch readout and decimal switch readout, 8 chars each,
    // left-padded to 16 chars to fill the row)
...
// =============== EDIT: ADDED NEW BLOCK OF CODE FROM BLUESPEC, MKOLEDTOP INSTANTIATION ===============
    ...
    assign str2 = {64'h2020202020202020, oledTop_binary_row};
    assign str4 = {64'h2020202020202020, oledTop_decimal_row};
...
// =================================== END OF ADDED BLOCK OF CODE ===================================
    ...
    // ================== ONLY EDIT FIRST LINE IF USING LEDS, MAKE SURE TO KEEP ONE FOR UPDATE READY ==================
    assign led = {update_ready, oledTop_leds[6:0]};     //lights up the leds via mkOledTop
    ...
    always@(posedge clk)
        case (state)
            ...
            // ==== EDIT: dBtnU/dBtnD BRANCHES CHANGED TO ALLOW RESAMPLING OF THE SWITCHES AND THE NUMBERS THAT WERE GENERATED ====
          
                 end else if (once == 1 && dBtnU == 1) begin
            // ==== Remove for now bc does not work
                    //str2 <= {64'h2020202020202020, oledTop_binary_row};// EDIT: snapshot latched here, before walk starts
            ...
            WriteWait: begin
                ...
                if (write_ready == 1'b1)
                    ...
                    // ====== EDIT: NEW IF/ELSE TO UPDATE THE VALUES GENERATED BY SWITCHES INSTEAD OF JUST IDLING IN ACTIVE ======
                    //    if (once == 1'b0) begin
                    //        once <= 1;
                    //        state <= Active;
                    //    end else begin
